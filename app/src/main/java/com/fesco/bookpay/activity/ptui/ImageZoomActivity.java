...
public class ImageZoomActivity extends AppCompatActivity implements DialogInterface.OnClickListener {
    ...
    private void initData() {
        ...
        boolean isHide = intent.getBooleanExtra(Constants.EXTRA_PREVIEW_IMG, false);
        if (isHide) {
    ...
    private void notificationRevokeChange(int location, String path) {
        ...
        EventBus.getDefault().postSticky(new RemoveImageEvent(location, true, path));
        // 判断是否是最后一张图片，如果是，则不进行图片数量的重置显示
        setTextNumber(location);
        adapter.notifyDataSetChanged();
    }
