...
public class MyProgessBar extends ProgressBar{
    ...
    private static final int DEFAULT_TEXT_OFFSET = 8;//文字两边的间距的总和
    private static final int DEFAULT_TEXT_DISTANCE =25;//文字上下的间距总和
    private static final int DEFAULT_TEXTBORDER_SIZE =2;//边框宽度
    private static final int DEFAULT_TEXTBORDER_COLOR=0xff14BAF1;

    private static final int DEFAULT_COLOR_RIGHT = 0xFF00b6d8;//文字右侧进度条颜色
    private static final int DEFAULT_HEIGHT_RIGHT  =4;//文字右侧进度条高度
    private static final int DEFAULT_COLOR_LEFT = 0xFF00b6d8;//文字左侧进度条颜色
    private static final int DEFAULT_HEIGHT_LEFT = 4;//文字左侧进度条高度
...
//            path.lineTo(leftProgressEndX + textWidth + mTextOffset + mTextBorderSize * 2 - middleTextBorderSize, mMaxContentHeight / 2 - mTextBorderSize);
      ...
      //  canvas.drawCircle(leftProgressEndX, -(mMaxContentHeight / 2 - middleTextBorderSize), 15, mPaint);
        canvas.drawCircle((textWidth + mTextOffset + mTextBorderSize * 2)/2 +leftProgressEndX , 0, 22, mPaint);
//x: (textWidth + mTextOffset + mTextBorderSize * 2)/2 +leftProgressEndX y:
        }
        /**
         * 画文字
         */
